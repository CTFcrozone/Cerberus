...
use lib_ebpf_common::consts::*;

fn module_op_to_string(op: u8) -> &'static str {
	match op {
		MODULE_OP_INIT => "INIT",
...
fn inode_op_to_string(op: u8) -> &'static str {
	match op {
...
fn inode_mutation_to_string(m: u8) -> &'static str {
	match m {
...
fn ptrace_stage_to_string(stage: u8) -> &'static str {
	match stage {
...
fn socket_op_to_string(op: u8) -> &'static str {
	match op {
		SOCKET_OP_BIND => "BIND",
...
pub fn line_from_event(evt: &CerberusEvent) -> Line<'static> {
	...
	match evt {
		...
		CerberusEvent::PtraceAccessCheck(p) => {
			let stage_str = match p.stage {
				0 => "REQUEST",
				1 => "DECISION",
				_ => "UNKNOWN",
			};
			...
			Line::raw(format!(
        "[PTRACE_{}] UID:{} | PID:{} | TGID:{} | CMD:{} -> TARGET_PID:{} | TARGET_TGID:{} | TARGET_UID:{} | TARGET_COMM:{} | MODE:{}",
        stage_str,
        h.uid,
        h.pid,
        h.tgid,
